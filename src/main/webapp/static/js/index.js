...
layui.use(['bodyTab','form','element','layer','jquery'],function(){
	var form = layui.form,
		...
		tab = layui.bodyTab({
			...
			url : "resource/findLeftMenu", //获取左边菜单json地址
		});

    //渲染左侧菜单
    tab.render();

	//隐藏左侧导航,以.hideMenu是类的引用，使用类来定位
...
//	});
	...
	function showNotice(){
		layer.open({
	        ...
	        shade: 0.8,
	        id: 'LAY_layuipro',
	        btn: ['快去看看，不要下次一定啦，各位看官行行好，点个双击加66666吧！！！！'],
	        moveType: 1,
	        content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p>本系统前端是通过layui + easyui集成实现的，后端基于ssm框架，数据库使用mysql数据库。</p></div>',
	         success: function(layero){
				...
				if($(window).width() > 432){  //如果页面宽度不足以显示顶部“系统公告”按钮，则不提示
					btn.on("click",function(){
						layer.tips('这是系统公告！！！！哈哈哈哈哈哈哈哈啊哈哈....', '#showNotice', {
							tips: 3,
                            time : 1000
						});
					})
